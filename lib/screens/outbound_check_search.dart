...
class _State extends State<OutboundCheckSearch> {
  String _id = '';
  bool _loading = true;
  ...
  void _submit() async {
    ...
    if (response.statusCode == 200) {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => OutboundCheckForm(_id))
      );
    } else {
      var jsonResponse = jsonDecode(response.body) as Map<String, dynamic>;
      String message = response.statusCode == 404
        ? "Delivery order data not found."
            ...
            Center(
              child: TextField(
                decoration: InputDecoration(
                  hintText: 'Input Delivery Order ID'
                ),
                onChanged: (value) => {
                  setState(() {
                    _id = value;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Outbound Check - Search Delivery Order'),
      ),
