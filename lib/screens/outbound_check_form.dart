...
class OutboundCheckForm extends StatefulWidget {
  final String _id;

  OutboundCheckForm(this._id);
  
  @override
  _State createState() => _State();
}

class _State extends State<OutboundCheckForm> {
  ...
  Future<GoodReceive> _getData([data = null]) async {
    ...
    if (data == null) {
      response = await http.get(Uri.parse(dotenv.env['API_URL'].toString() + '/do_check/' + _id + '/data'));
    } else {
      response = data;
    }
  ...
  void _submitData(context) async {
    ...
    final response = await http.post(
      Uri.parse(dotenv.env['API_URL'].toString() + '/do_check/' + _id + '/check'),
      headers: {
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData && !_loading) {
          return DefaultTabController(
            ...
            child: Scaffold(
              appBar: AppBar(
                title: Text('Outbound Item Check'),
                bottom: TabBar(
                  tabs: const [
                    Tab(
                      child: Text('Info')
